...
//  project1
//
//  Created by Kashita Talreja on 23/12/20.
//subset problem -DP1
#include <bits/stdc++.h>
using namespace std;
...
void solve(){
    ...
    for(int i=1;i<=n;i++){
        for(int j=1;j<=w;j++){
            if(a[i-1]<=j) // wt of element is less than temp target weight
                t[i][j]=t[i-1][j-a[i-1]] || t[i-1][j]; // if any of them is true, include|| dont include
            else t[i][j]=t[i-1][j];
        }
    }
    cout<<t[n][w]<<endl;
